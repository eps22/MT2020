ThermalSymmB <- function(compfolder, currentimeidx, consecutivecenters, resolution, Blowerpressurelevel, Bupperpressurelevel, Bmultiplemeasure, Bdirections, zerovortradius2, PressureVertLevelDimName, ZVarName){
    ...
    hpaHIGHidx <- which(plevs==Blowerpressurelevel, arr.ind = TRUE)
    hpaLOWidx <- which(plevs==Bupperpressurelevel, arr.ind = TRUE)
