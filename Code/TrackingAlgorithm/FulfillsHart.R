FulfillsHart <- function(compfolder, currentimeidx, consecutivecenters, resolution, HartConditionsTocheck, Blowerpressurelevel, Bupperpressurelevel, Bmultiplemeasure, Bdirections, Bthreshold, LTWlowerpressurelevel, LTWupperpressurelevel, UTWlowerpressurelevel, UTWupperpressurelevel, zerovortradius2){
  
  conditions <- c()
  values <- c()
  
  if((1 %in% HartConditionsTocheck)==TRUE){
    Bparam <- ThermalSymmB(compfolder, currentimeidx, consecutivecenters, resolution, Blowerpressurelevel, Bupperpressurelevel, Bmultiplemeasure, Bdirections, zerovortradius2)
  ...
  if((1 %in% HartConditionsTocheck)==TRUE && BSymmetric==TRUE){
    ...
    if((2 %in% HartConditionsTocheck)==TRUE && (3 %in% HartConditionsTocheck)==TRUE){
      TW <- ThermalWind(compfolder, currentimeidx, consecutivecenters, resolution, LTWlowerpressurelevel, LTWupperpressurelevel, UTWlowerpressurelevel, UTWupperpressurelevel, zerovortradius2, lowerTW=TRUE, upperTW=TRUE)
    ...
    if((2 %in% HartConditionsTocheck)==TRUE && (3 %in% HartConditionsTocheck)==FALSE){
      TW <- ThermalWind(compfolder, currentimeidx, consecutivecenters, resolution, LTWlowerpressurelevel, LTWupperpressurelevel, UTWlowerpressurelevel, UTWupperpressurelevel, zerovortradius2, lowerTW=TRUE, upperTW=FALSE)
    ...
    if((2 %in% HartConditionsTocheck)==FALSE && (3 %in% HartConditionsTocheck)==TRUE){
      TW <- ThermalWind(compfolder, currentimeidx, consecutivecenters, resolution, LTWlowerpressurelevel, LTWupperpressurelevel, UTWlowerpressurelevel, UTWupperpressurelevel, zerovortradius2, lowerTW=FALSE, upperTW=TRUE)
      conditions <- c(conditions, TW$LTW>0)  
