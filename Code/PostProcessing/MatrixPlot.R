MatrixPlot <- function(field, lon, lat, crs, resolution = NULL, dthreshold = NULL, dtthreshold = NULL,
  ...
  if(is.null(dim(lat)) | length(dim(lat))==1){lat <- t(matrix(rep(latsc, times=length(latsc)),nrow=length(latsc),ncol=length(lonsc)))}
  rm(lonsc,latsc)
  ...
  if(crs==crsLONGLAT){
    df <- data.frame(x = c(lon), y = c(lat), z = c(field))
    
    #Interpolate data to prevent empty points due to projection
    fld <- with(df, interp(x = x, y = y, z = z, duplicate ="mean", nx=dim(field)[1], ny=dim(field)[2]))
