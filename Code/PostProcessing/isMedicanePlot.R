...
directory <- '/media/q/Seagate Backup Plus Drive/ACEX/Medicanes/outputprocessed'
...
chemnochem <- 'WRF-chem' #'WRF' or 'WRF-chem'. Only used when fix='chem'
...

{
  ...
  if(Event=='Celeno'){
    ...
    initialdate <- '1995-01-13 00:00:00' #Celeno
  ...
  if(Event=='Numa'){
    ...
    initialdate <- '2017-11-15 00:00:00'
  ...
  for(dir in dirs1){
    ...
    file <- paste(directory,'/',Event,'/',dir,'/',dir,'-trackdf.csv',sep="")
    ...
    for(i in 1:maxtime){
      if(i %in% data0$timestep){
        ...
        if(length(selectindex)==1){
          data <- rbind(data, as.numeric(data0[selectindex,]))
        } else{
  ...
  casenames <- sapply(refslistt1, function(x) strsplit(strsplit((strsplit(x,'to')[[1]][1]),paste('-',medicaneyear,'-',sep=''))[[1]][2],'-')[[1]][2], USE.NAMES=FALSE)
